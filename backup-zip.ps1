# NAME: 💾 Backup Project
# DESCRIPTION: Creates clean backup using 7-Zip in .zip format (keeps last 20)
...
$backup = "$backupFolder\$(Get-Date -Format 'yy-MM-dd_HHmm').zip"
...
# Create archive (using -tzip to create a .zip file)
& $7z a -tzip $backup $source $exclude
...
$backups = Get-ChildItem -Path $backupFolder -Filter "*.zip" | Sort-Object CreationTime -Descending
...
Get-ChildItem -Path $backupFolder -Filter "*.zip" | Sort-Object CreationTime -Descending | ForEach-Object {
